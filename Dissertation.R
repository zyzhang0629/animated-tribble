...
ggplot(data_combined, aes(x = Nitrogen.dioxide, fill = Site)) +
  geom_histogram(binwidth = 1, alpha = 0.7, position = "identity") +
  facet_wrap(~ Site, scales = "free_y") +
  labs(title = "Histogram of Nitrogen Dioxide Levels at Three Sites",
       x = "Nitrogen Dioxide (µg/m³)",
       y = "Frequency") +
  theme_minimal()

ggplot(data_combined, aes(x = Nitric.oxide, fill = Site)) +
  geom_histogram(binwidth = 1, alpha = 0.7, position = "identity") +
  facet_wrap(~ Site, scales = "free_y") +
  labs(title = "Histogram of Nitric Oxide Levels at Three Sites",
       x = "Nitric Oxide (µg/m³)",
       y = "Frequency") +
  theme_minimal()

ggplot(data_combined, aes(x = Nitrogen.oxides.as.nitrogen.dioxide, fill = Site)) +
  geom_histogram(binwidth = 1, alpha = 0.7, position = "identity") +
  facet_wrap(~ Site, scales = "free_y") +
  labs(title = "Histogram of Nitrogen Oxides as Nitrogen Dioxide Levels at Three Sites",
       x = "Nitrogen Oxides as Nitrogen Dioxide (µg/m³)",
       y = "Frequency") +
  theme_minimal()


ggplot(data_combined, aes(x = PM10.particulate.matter..Hourly.measured., fill = Site)) +
  geom_histogram(binwidth = 1, alpha = 0.7, position = "identity") +
  facet_wrap(~ Site, scales = "free_y") +
  labs(title = "Histogram of PM10 Particulate Matter Levels at Three Sites",
       x = "PM10 Particulate Matter (µg/m³)",
       y = "Frequency") +
  theme_minimal()

ggplot(data_combined, aes(x = PM2.5.particulate.matter..Hourly.measured., fill = Site)) +
  geom_histogram(binwidth = 1, alpha = 0.7, position = "identity") +
  facet_wrap(~ Site, scales = "free_y") +
  labs(title = "Histogram of PM2.5 Particulate Matter Levels at Three Sites",
       x = "PM2.5 Particulate Matter (µg/m³)",
       y = "Frequency") +
  theme_minimal()
#all Right skewed distribution
#Using square root transformation
data_Dewsbury_filtered <- data_Dewsbury_filtered %>%
  mutate(across(c(Nitric.oxide, Nitrogen.dioxide, Nitrogen.oxides.as.nitrogen.dioxide, 
                  PM10.particulate.matter..Hourly.measured., 
                  PM2.5.particulate.matter..Hourly.measured.), sqrt))
...
data_combined <- rbind(data_Dewsbury_filtered, data_LeedsCentre_filtered, data_Headingly_filtered)
ggplot(data_combined, aes(x = sqrt(Nitrogen.dioxide), fill = Site)) +
  geom_histogram(binwidth = 0.1, alpha = 0.7, position = "identity") +
  facet_wrap(~ Site, scales = "free_y") +
  labs(title = "Square Root Transformed Histogram of Nitrogen Dioxide Levels at Three Sites",
       x = "Square Root(Nitrogen Dioxide) (sqrt(µg/m³))",
       y = "Frequency") +
  theme_minimal()
...
data_combined$Time <- format(as.POSIXct(data_combined$Time, format="%H:%M:%S"), "%H:%M:%S")
data_combined$Hour <- as.numeric(format(as.POSIXct(data_combined$Time, format="%H:%M:%S"), "%H"))

custom_colors <- c("Dewsbury_Ashworth_Grange" = "#FF5733", 
                   "Leeds_Centre" = "#33FF57",
                   "Leeds_Headingly_Kerbside" = "#3357FF") 
plot_hourly_avg <- function(data, pollutant, y_label) {
  hourly_avg <- data %>%
    group_by(Site, Hour) %>%
    summarise(Avg = mean(get(pollutant), na.rm = TRUE))
  
  hourly_avg <- hourly_avg[order(hourly_avg$Hour),]
  
  ggplot(hourly_avg, aes(x = Hour, y = Avg, color = Site, group = Site)) +
    geom_line(size = 1.5) +
    labs(title = paste("24-Hour Average", y_label, "Levels at Three Sites"),
         x = "Hour of Day",
         y = paste("Average", y_label, "(sqrt(µg/m³))")) +
    theme_minimal() +
...
pollutants <- list(
  ...
  Nitrogen.oxides.as.nitrogen.dioxide = "Nitrogen Oxides as Nitrogen Dioxide",
  PM10.particulate.matter..Hourly.measured. = "PM10 ",
  PM2.5.particulate.matter..Hourly.measured. = "PM2.5 "
)

plots <- lapply(names(pollutants), function(pollutant) {
  plot_hourly_avg(data_combined, pollutant, pollutants[[pollutant]])
})
